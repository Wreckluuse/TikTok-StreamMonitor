...
<div transition:fly={{y: 25, duration: 300, easing: quintInOut}}
	...
	class="bg-primary overflow-x-hidden scrollbar-thin scrollbar-rounded-lg scrollbar-thumb-primary scrollbar-track-secondary  rounded-sm h-full w-full shadow"
>
	{#if settingsOpen}
		<div
			class="inline grid grid-cols-4 grid-gap-1 z-10 w-full h-[4%] bg-accent shadow text-sm rounded-t-sm"
	...
	<div class="inline h-[4%] flex flex-cols w-full rounded-t-sm">
		<div
			class="inline bg-accent pr-5 place-items-left text-2xl text-zinc-100 w-full z-5 shadow rounded-t-sm"
		>
			Chat
		</div>
